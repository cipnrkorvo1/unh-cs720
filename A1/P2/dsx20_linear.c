...
int main(int argc, char* argv[])
{
    ...
    if (argc != 2)
    {
        printf("Usage: dsx20 <object file>\n");
        exit(1);
    }
    ...
    // print object code
    printf("\nObject Code (%d words)\n\n", obj_len);
    read_code(3+(insym_count*5)+(outsym_count*5), obj_len);
...
void read_code(WORD32 start, int count)
{
    WORD32* ptr = (WORD32*)fbuf + start;
    for (int pc = 0; pc < count; pc++)
    {
        ...
        switch (format) {
            ...
            case F_REG:
                ...
                break;
            case F_REGCONST:
                printf("r%d, %d", (cur >> 8) & 0xf, (int32_t)((cur >> 12) & 0xfffff));
                break;
            case F_REGADDR:
                addr = EXTENDBYTE20(cur >> 12);
                printf("r%d, ", (cur >> 8) & 0xf);
